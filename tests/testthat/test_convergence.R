...
model <- findCNVs(file, ID='test', eps=0.1, most.frequent.state='2-somy', states=c("zero-inflation",paste0(1:9,'-somy'),'+10-somy'), num.trials=1)
w <- model$weights
...
# With 0-somy
file <- list.files(pattern='euploid_')
model <- findCNVs(file, ID='test', eps=0.1, most.frequent.state='2-somy', states=c("zero-inflation",paste0(0:9,'-somy'),'+10-somy'), num.trials=1)
w <- model$weights
expect_that(w['2-somy'], is_more_than(0.85))
expect_that(w['2-somy'], is_less_than(0.91))
