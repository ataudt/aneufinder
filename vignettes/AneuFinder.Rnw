...
After the function has finished, you will find the folder $<$output-directory$>$ containing all produced files and plots. This folder contains the following \emph{files} and \textbf{folders}:
\begin{itemize}
...
\item \textbf{browserfiles\_data}: This folder contains BED files with mapped reads that can be uploaded to the UCSC genome browser. The BED files contain the same reads as your data but filtered by mapping quality and other parameter settings that you can find in section [Binning] of the ``AneuFinder.config'' file. Set option reads.store=FALSE to omit production of this directory and speed up calculation.
\item \textbf{browserfiles}: A folder which contains BED files with CNV calls that can be uploaded to the UCSC genome browser.
\item \textbf{data}: This folder stores all the read data as RData objects. This exists mostly for internal usage. Set option reads.store=FALSE to omit production of this directory and speed up calculation.
\item \textbf{hmms}: A folder with all produced Hidden Markov Models. You can load the results for further processing, such as quality control and customized plotting.
...
## Cluster by quality
cl <- clusterByQuality(files, measures=c('spikiness','num.segments','entropy','bhattacharyya'))
