...
#' 
#' @param files A list of either BAM or BED files.
#' @param format The format of \code{files}. Either 'bam' or 'bed'.
...
blacklist <- function(files, format, assembly, bins, min.mapq=10, pairedEndReads=FALSE) {
	...
	for (file in files) {
		dup.binned <- binReads(file, format = format, assembly = assembly, min.mapq = min.mapq, pairedEndReads = pairedEndReads, binsizes = NULL, bins = bins, chromosomes = seqlevels(bins[[1]]), calc.complexity = FALSE, remove.duplicate.reads = FALSE)
		dupcounts.matrix[,basename(file)] <- dup.binned[[1]]$counts
		binned <- binReads(file, format = format, assembly = assembly, min.mapq = min.mapq, pairedEndReads = pairedEndReads, binsizes = NULL, bins = bins, chromosomes = seqlevels(bins[[1]]), calc.complexity = FALSE, remove.duplicate.reads = TRUE)
